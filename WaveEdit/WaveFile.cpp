...
// Get ith sample
int 
WaveFile::get_sample(int i) { // For some reason crashes
    short val;
    if (isLittleEndian()) {
...
WaveFile *
WaveFile::changeSpeed(float speed){ // May add in selection handling?
	...
	while(r < (this->lastSample)){
		i = (int)r;
		newWave->add_sample(get_sample((int)i));
		r = r + speed;
		j++;
	}
...
{
	//TODO:
	// Copy the data at start to end into the clipboard
	WaveFile * newWave = new WaveFile(numChannels, sampleRate, bitsPerSample);

	for(int a = (startMs * (this->sampleRate / 1000)); a < (endMs * (this->sampleRate / 1000)); a++){
...
WaveFile::remove_fragment(int startMs, int endMs)
{
	...
	for(int a = (endMs * (this->sampleRate / 1000)); a < this->lastSample; a++){
		newWave->add_sample(this->get_sample(a));	
	}
...
WaveFile * 
WaveFile::paste_fragment(int startMs, WaveFile * clipping) //Doesn't work across documents
{
	WaveFile * newWave = new WaveFile(numChannels, sampleRate, bitsPerSample);
	...
	for(int c = (startMs * (this->sampleRate / 1000)); c < this->lastSample; c++){
...
WaveFile * 
WaveFile::replace_fragment(int startMs, int endMs, WaveFile * clipping) //Doesn't work across documents
{
	WaveFile * newWave = new WaveFile(numChannels, sampleRate, bitsPerSample);
...
WaveFile::shift(int modulation)
{
	...
	for(int x = 0; x < this->lastSample; x++){
		newWave->add_sample(this->get_sample(x) + modulation);
	}
